...
def test_distance_known_points_small():
    ...
    # Expected distance from online calculator (~1.3 km)
    expected_distance = 1.34 # km
...
def test_distance_known_points_larger():
    ...
    # Expected distance from online calculator (~5570 km)
    expected_distance = 5570.2 # km
    assert np.isclose(distance(origin, destination), expected_distance, atol=0.1)
...
def test_nearest_point_empty_latpoints_1d():
    ...
    with pytest.raises(ValueError, match="zero-size array to reduction operation min which has no identity"):
