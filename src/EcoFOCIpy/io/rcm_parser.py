"""
Contains a collection of RCM equipment parsing.
...
class rcm_excel(object):
    ...
    def parse_excel(filename=None, datetime_index=True):
        ...
        rawdata_df = pd.read_excel(
            filename, skiprows=4, parse_dates=["date/time"], index_col="date/time"
        )
        ...
        if datetime_index:
            ...
            ).drop(["date_time", "date/time"], axis=1)
...
class rcm(object):
    ...
    def parse(
        self, filename=None, number_of_channels=8, time_format=0, datetime_index=True
    ):
        r"""
        ...
        if time_format == 0:
            rawdata_df["date_time"] = pd.to_datetime(
                rawdata_df["date"] + " " + rawdata_df["time"],
                format="%m/%d/%Y %H:%M:%S",
            )
        elif time_format == 1:
            rawdata_df["date_time"] = pd.to_datetime(
            ...
            pass
        rawdata_df["temp_engr"] = pd.to_numeric(
            rawdata_df["temp_engr"], errors="coerce"
        )
    ...
    def drop_headerrows(self, ident=None):
        ...
        """
        self.rawdata_df = self.rawdata_df[
            self.rawdata_df["ident"] == ident
        ]  # <-- there is a value that represents data and one that represents headers, drop all the headers
    ...
    def engr2sci_curr(self):
        ...
        self.rawdata_df["current_direction_uncorrected"] = (
            self.rawdata_df["dir_engr"] * 0.3515
        )
        self.rawdata_df["u_curr_comp"] = self.rawdata_df["current_speed"] * np.sin(
            np.deg2rad(self.rawdata_df["current_direction_uncorrected"])
        )
        self.rawdata_df["v_curr_comp"] = self.rawdata_df["current_speed"] * np.cos(
            np.deg2rad(self.rawdata_df["current_direction_uncorrected"])
        )
    ...
    def engr2sci_temp(self, coefA=0, coefB=0, coefC=0, coefD=0):
        ...
        """
        self.rawdata_df["temperature"] = (
            coefA
            + coefB * self.rawdata_df["temp_engr"]
            + +coefC * (self.rawdata_df["temp_engr"] ** 2)
            + coefD * (self.rawdata_df["temp_engr"] ** 3)
        )
    ...
    def engr2sci_pres(self, coefA=0, coefB=0, coefC=0, equationType="low", units="kPa"):
        ...
        """
        if equationType == "low":
            self.rawdata_df["pressure"] = (
                coefA
                + coefB * self.rawdata_df["press_engr"]
                + coefC * (self.rawdata_df["press_engr"] ** 2)
            )
            if units == "MPa":
    ...
    def mag_dec_corr(self, lat, lonW, dep_date, apply_correction=True):
        ...
        (u, v) = geotools.rotate_coord(
            self.rawdata_df["u_curr_comp"], self.rawdata_df["v_curr_comp"], dec
        )
...
class rcm_sg(object):
    ...
    These datastreams are no longer binary off the instrument and can be qc'd by the aandera
    software This means that a preliminary qc'd product is archived in netcdf via output files.

    Due to the Seaguard being a platform, multiple datafiles are possible, usually including
    oxygen but not always
    ...
    def parse_excel(filename=None, datetime_index=True):
        ...
        try:
            rawdata_df["date_time"] = pd.to_datetime(
                rawdata_df["Time tag (Gmt)"], format="%d.%m.%y %H:%M:%S"
            )
        except KeyError:
            rawdata_df["date_time"] = pd.to_datetime(
                rawdata_df["Record Time"], format="%m/%d/%y %H:%M:%S"
            )
    ...
    def parse(filename=None, datetime_index=True):
        ...
        rawdata_df = pd.read_csv(
            filename, delimiter="\t", parse_dates=True, skiprows=headercount
        )
        try:
            rawdata_df["date_time"] = pd.to_datetime(
                rawdata_df["Time tag (Gmt)"], format="%d.%m.%Y %H:%M:%S"
