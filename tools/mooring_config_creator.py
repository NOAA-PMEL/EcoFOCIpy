...
parser = argparse.ArgumentParser(
    description="Mooring Meta Builder - outputs a parsed text \
                                              of meta information unless the yaml flag is provided"
)
parser.add_argument(
    "MooringID", metavar="MooringID", type=str, help="MooringID of form 13BSM-2A"
)
parser.add_argument(
    "-db",
    "--db_ini",
...
parser.add_argument(
    "-yaml", "--yaml_format", action="store_true", help="format for yaml files"
)
...
Mooring_Meta_inst = EcoFOCI_db.read_mooring_inst(
    table=table, mooringid=args.MooringID, verbose=True
)
table = "mooringdeploymentlogs"
Mooring_Meta_sum = EcoFOCI_db.read_mooring_summary(
    table=table, mooringid=args.MooringID, verbose=True
)
table = "mooringrecoverylogs"
Mooring_recovery_sum = EcoFOCI_db.read_mooring_summary(
    table=table, mooringid=args.MooringID
)
table = "mooringdeploymentnotes"
Mooring_Meta_notes = EcoFOCI_db.read_mooring_summary(
    table=table, mooringid=args.MooringID
)
...
try:
    ...
    print(
        "No known mooring {0}.  Check syntax and case (e.g. 14BSM-2A)".format(
            args.MooringID
        )
    )
    sys.exit()
...
if args.yaml_format:
    ...
    try:
        data_dic["Recovery"] = {
            "RecoveryCruise": Mooring_recovery_sum[args.MooringID]["CruiseNumber"],
            ...
            "RecoveryDateTimeGMT": Mooring_recovery_sum[args.MooringID][
                "RecoveryDateTimeGMT"
            ],
    ...
    try:
        for instrument in sorted(Mooring_Meta_inst.keys()):
            InstrumentDic[Mooring_Meta_inst[instrument]["InstID"]] = {
                "InstType": Mooring_Meta_inst[instrument]["InstType"],
                ...
                "PreDeploymentNotes": Mooring_Meta_inst[instrument][
                    "PreDeploymentNotes"
                ],
                "PostDeploymentNotes": Mooring_Meta_inst[instrument][
                    "PostDeploymentNotes"
                ],
    ...
    print(
        ("Deployment Depth:\t{0}m\n").format(
            Mooring_Meta_sum[args.MooringID]["DeploymentDepth"]
        )
    )
    print(
        ("DeploymentDateTimeGMT:\t{0}\n").format(
            Mooring_Meta_sum[args.MooringID]["DeploymentDateTimeGMT"]
        )
    )
    print(
        ("RecoveryDateTimeGMT:\t{0}").format(
            Mooring_recovery_sum[args.MooringID]["RecoveryDateTimeGMT"]
        )
    )
...
----------\t-------\t\t-----\t\t-----\t\t----------\t\t------
    ...
    for instrument in sorted(Mooring_Meta_inst.keys()):
        ...
        print(
            ("{0}\t\t{1}\t\t{2}\t\t{3}\t\t{4}\t\t{5}").format(
                Mooring_Meta_inst[instrument]["InstType"],
                Mooring_Meta_inst[instrument]["SerialNo"],
                Mooring_Meta_inst[instrument]["Depth"],
                Mooring_Meta_inst[instrument]["ActualDepth"],
                Mooring_Meta_inst[instrument]["DataStatus"],
                Mooring_Meta_inst[instrument]["PostDeploymentNotes"],
            )
        )
