...
def test_latlon_convert_north_east():
    ...
    lon_str = "075 45.00 E"
    expected_lat = 30.0
    expected_lon = (75 + 45 / 60.0)  # West is negative
    lat, lon = latlon_convert(lat_str, lon_str)
    assert np.isclose(lat, expected_lat)
    assert np.isclose(lon, expected_lon)
...
def test_latlon_convert_south_east():
    ...
    lat_str = "05 05.00 S"
    lon_str = "170 30.00 E"
    expected_lat = -(5 + 5 / 60.0)
    expected_lon = (170 + 30 / 60.0)
    lat, lon = latlon_convert(lat_str, lon_str)
    assert np.isclose(lat, expected_lat)
    assert np.isclose(lon, expected_lon)
...
def test_rotate_coord_all_nan_values():
    ...
    assert np.isclose(uu[0], np.nan, equal_nan=True)
    assert np.isclose(vv[0], np.nan, equal_nan=True)
