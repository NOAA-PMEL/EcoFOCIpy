...
def test_get_filepath_no_path_no_dir_raises_value_error(adcp_instance_no_dir):
    with pytest.raises(
        ValueError,
        match="Must provide either a deployment directory or a direct file path.",
    ):
...
def test_get_filepath_file_not_found_raises_file_not_found_error(adcp_instance):
    with pytest.raises(
        FileNotFoundError, match="The specified ADCP file does not exist"
    ):
...
def test_load_rpt_file_missing_params(tmp_path):
    ...
    (tmp_path / f"{serial_no}.RPT").write_text("ADCP Report\n" "Some other line\n")
...
@pytest.mark.skipif(
    not ECOFOCIPY_AVAILABLE, reason="EcoFOCIpy not installed, skipping geomag tests"
)
def test_mag_dec_corr_integration(adcp_instance):
    ...
    declination = adcp_instance.mag_dec_corr(
        lat=60.0, lon_w=150.0, deployment_date=pd.Timestamp("2023-01-01")
    )
...
@pytest.mark.parametrize(
    "u, v, declination, expected_u, expected_v",
    [
...
def test_mag_dec_corr_mocked_geotools(
    mocker, adcp_instance, u, v, declination, expected_u, expected_v
):
    """
    ...
    mock_rotate_coord = mocker.patch(
        "adcp_parser.geotools.rotate_coord",
    ...
    adcp_instance.mag_dec_corr(
        lat=0, lon_w=0, deployment_date=pd.Timestamp("2023-01-01")
    )
    ...
    np.testing.assert_allclose(
        adcp_instance.vel_df["u_curr_comp"].iloc[0], expected_u, rtol=1e-5
    )
    np.testing.assert_allclose(
        adcp_instance.vel_df["v_curr_comp"].iloc[0], expected_v, rtol=1e-5
    )
...
def test_mag_dec_corr_raises_value_error_if_no_vel_df(adcp_instance_no_dir):
    ...
    with pytest.raises(ValueError, match="Velocity data must be loaded"):
        adcp_instance_no_dir.mag_dec_corr(
            lat=0, lon_w=0, deployment_date=pd.Timestamp("2023-01-01")
        )
